...
void PlayScene::update()
{
	...
	float dt = 0.0125;//Game::Instance().getDeltaTime();
	m_gameTime += dt;
	...
	for (auto it = projectiles.begin(); it != projectiles.end(); it++)
	{
		...
		// if ship faces ground - it will not move down (velocity = 0)
		sprite->getTransform()->position.y > groundPosition ?
			sprite->getRigidBody()->velocity.y = 0 : sprite->getRigidBody()->velocity.y += accelerationGravity * massKg * dt;
...
void PlayScene::GUI_Function()
{
	...
	ImGui::SliderFloat("mass", &massKg, 0.0f, 50.0f, "%.1f,");
